		...
		switch (types) {
			case "Amenities":
				return (
					<>
						{anchor && (
							<a id={anchor} name={anchor} className="anchor"></a>
						)}
						<div className={`${styles.sliderContent} sliderfadein`}>
							<p className="sans-serif-bold sub-heading white">
								{title}
							</p>
							{blurb && (
								<p
							...
							{description && !isMobileDevice && (
								<p className="sans-serif body-copy white desc">
									{description}
								</p>
							)}
							<p
			...
			case "Events":
				return (
					<>
						...
						<div
							className={`${styles.sliderNavEvents} sliderfadein`}>
							{amenities.map((amenity, index) => {
								return (
									<p
										key={`${amenity.title}-nav`}
