          ...
          <>
            ...
            <div className={`${styles.sliderContent} sliderfadein`}>
              ...
              {description && !isMobileDevice && (
                <p className="sans-serif body-copy white desc">{description}</p>
              )}
              <p
              ...
              >
                {amenities.map((item, index) => {
                  return (
                    ...
                    >
                      <p
                        className={`${styles.mobileTitle} serif heading white left textshadow`}
                      >
                        <a href={item.amenityContent?.sliderLink}>
                          {item.title}
                        </a>
                      </p>
                      {!isMobileDevice && (
                        <Image
                          src={
                            item.amenityContent?.mobileSliderImage?.mediaItemUrl
                          }
                          alt={item.amenityContent?.mobileSliderImage?.altText}
                          width={856}
                          height={890}
                          layout="intrinsic"
                          className={styles.mobileSliderImg}
                        />
                      )}
                    </div>
                  );
            ...
            <div className={`${styles.sliderNav} sliderfadein`}>
              {amenities.map((amenity, index) => {
                return (
                  <p
                    key={`${amenity.title}-nav`}
                    className={`${sliderActive == index ? styles.active : ""}`}
                  >
                    <a
