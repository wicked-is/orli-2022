	...
	return (
		<section className={`${styles.historicTimelineContainer} timelineSliderglobal`} aria-labelledby={titleId}>
			<Swiper
				...
				{years.map((year, index) => {
					...
					return (
						<SwiperSlide
							key={year.year}
							className={`${styles.year} ${styles.singleyear}`}
							style={{ backgroundImage: `url(${year.image.mediaItemUrl})` }}>
							...
							<div className={styles.yearTextContainer}>
								<div className={styles.yearContentContainer}>
									<h2 className={`xl-heading ${styles.actualyear} center`} data-text={previousYear} style={{ lineHeight: 1 }}>
										{year.year}
									</h2>
									<p
										className="sans-serif-bold uppercase"
										style={{ letterSpacing: "var(--letter-spacing)" }}
										dangerouslySetInnerHTML={{ __html: year.caption }}></p>
								</div>
							</div>
						</SwiperSlide>
					);
			...
			<div className={styles.sliderNavContainer}>
				<h2 id={titleId} className={`serif heading white ${styles.sliderTitle}`}>
					Building History
				</h2>
					...
					{years.map((year, index) => {
						return (
							<p key={`${year.year}-nav`} className={`${sliderActive == index ? styles.active : ""}`}>
								<a
									className={`${styles.navItem} ${sliderActive == index ? styles.active : ""}`}
									role="button"
									tabIndex={0}
								...
								<span
									className={`sans-serif-bold uppercase ${styles.mobiledescription} ${sliderActive == index ? styles.active : ""}`}
									dangerouslySetInnerHTML={{ __html: year.caption }}></span>
							</p>
						);
					})}
				</div>
