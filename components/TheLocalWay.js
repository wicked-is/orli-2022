...
export default function TheLocalWay(props) {
	...
	useEffect(() => {
		...
		function initMap() {
			map = new google.maps.Map(document.getElementById("map"), {
				center: { lat: 32.845857, lng: -117.26831 },
				zoom: 14.7,
	...
	return (
		<TheLocalWayContainer>
			<Script
				id="google-mmaps"
				src={`https://maps.googleapis.com/maps/api/js?key=${process.env.GMAP_API}&callback=initMap&v=weekly`}
						...
						<Column key={main} className={isActivethree === main ? "activecol" : null} onClick={() => toggleClassthree(main)}>
							{column.sections.map((section, index) => {
								return (
									<ColumnSection
										key={index}
										className={`columnnumber ${isActivetwo === index ? "activecoltwo" : null}`}
										onClick={() => toggleClasstwo(index)}>
										<h2 onClick={() => toggleShow(index)} className="sans-serif sub-heading-bold black">
											{section.title}
										</h2>
										<ListContainer className={`listcontainer ${isShow === index ? "showme" : null}`}>
											{section.locations &&
												section.locations.map((location, i, index) => (
													<p
														className={isActive === i ? "activestate" : null}
														key={i}
