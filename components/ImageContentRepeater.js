...
export default function ImageContentRepeater(props) {
	const { anchor, paddingOptions, heading, smallHeading, repeater } = props;

	return (
		<ImageContentContainer className={paddingOptions}>
			{anchor && <a id={anchor}></a>}
			{smallHeading && (
				<h3 className="sans-serif-bold left">{smallHeading}</h3>
			)}
						...
						return (
							<MediaContent
								...
								<div className="content">
									{item.title && (
										<h2 className="sans-serif left">
											{item.title}
										</h2>
									)}
									{item.subheading && (
										<h3 className="serif left">
											{item.subheading}
										</h3>
									)}
